	...
	)
{
	...
	uint32	i;
	uint32  j; 			/*iterator for initializing the message buffer states DMP */
	void	INITRET(void);
	//struct memblkgb ref_head; 	/* head of memory reference list DMP */
	...
	/* Set up the message buffer states used by sendb and receiveb DMP */
	for(j = 0; j < MSGBUFSIZE; j++) {
		prptr->prmsgstate[j] = MSGEMPTY;
	}

	/* Set up the send queue of the process DMP */
	prptr->sendqueue = newqueue();

	/* Initialize the has_register_recv flag to false DMP */
	prptr->has_register_recv = FALSE;

	/* Initialize the memory reference list DMP */
	//ref_head.mnext = NULL;
	//ref_head.mlength = 0;
	//ref_head.heap_address = NULL;

	prptr->mem_reference_head = NULL;
	//prptr->mem_reference_head = &ref_head;
	//prptr->mem_reference_head->mnext = NULL;
	//prptr->mem_reference_head->mlength = 0;
	//prptr->mem_reference_head->heap_address = NULL;
