	...
	)				/*  to accept commands		*/
{
	...
	while (TRUE) {
		...
		pid32 	searchProc;
		int32 	searchArg;
		int32 	searchArgIndex[2] = {0, 7};
		char 	searchArgBuf[9];
		char c;
		int stct;
	
		//if both gen and search are called w/o a pipe
		if( (ntok == 2) && !strncmp("gen", &tokbuf[tok[0]], strnlen(&tokbuf[tok[0]], 3)) && !strncmp("search", &tokbuf[tok[2]], strnlen(&tokbuf[tok[2]], 6))) {
			kprintf("Syntax Error\n");
			continue;
		}

		//if two programs are ran with a pipe
		if( (ntok == 3) && toktyp[1] == SH_TOK_PIPE)  {

			//if the the commands are gen and search
			if(!strncmp("gen", &tokbuf[tok[0]], strnlen(&tokbuf[tok[0]], 3)) && !strncmp("search", &tokbuf[tok[2]], strnlen(&tokbuf[tok[2]], 6))) {

				//create two processes for gen and search
				genProc = create(cmdtab[14].cfunc, SHELL_CMDSTK, SHELL_CMDPRIO, "xsh_gen", 2, 2, &genArg);
				searchProc = create(cmdtab[15].cfunc, SHELL_CMDSTK, SHELL_CMDPRIO, "xsh_search", 2, 2, &searchArg);
				...
				//connect the pipe
				if(pipconnect(pipe, genProc, searchProc) == SYSERR) {
					return SYSERR;
				}
		

				//format the arguments to be passed to gen and search
				strncpy(genArgBuf, "gen\0", 4);
				sprintf(pipeidstr, "%d", pipe);
				c = pipeidstr[0];
				stct = 0;
				while(c != 0 && (stct < strnlen(pipeidstr, 20))) {
					strncpy(genArgBuf + 4 + stct, &pipeidstr[stct], 1);
					stct++;
				}

				genArgBuf[5] = '\0';	
				
				strncpy(searchArgBuf, "search\0", 7);
				c = pipeidstr[0];
				stct = 0;
				while(c != 0 && (stct < strnlen(pipeidstr, 20))) {
					strncpy(searchArgBuf + 7 + stct, &pipeidstr[stct], 1);
					stct++;
				}
				searchArgBuf[8] = '\0';

				//add the arguments for gen and search
				addargs(genProc, 2, genArgIndex, 25, genArgBuf, &genArg);
				addargs(searchProc, 2, searchArgIndex, 28, searchArgBuf, &searchArg);
